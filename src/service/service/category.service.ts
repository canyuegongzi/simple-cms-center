...
export class CategoryService {
  ...
  public async getList(query: QueryCategoryDtoDto): Promise<any> {
    try {
      const res = await this.categoryRepository
          ...
          .orWhere('c.name like :name', { name: `%${query.name}%`})
          .orderBy('c.name', 'ASC')
  ...
  public async creatCategory(params: CreateCategoryDto) {
    try {
      const currentCategory: Category = await this.categoryRepository.findOne({id: params.parentId});
      if (!currentCategory) {
      ...
      return await this.categoryRepository
          ...
          .insert()
          .into(Category)
          .values([{name: params.name, code: params.code, desc: params.desc, parentId: params.parentId, sort: params.sort}])
          .execute();
    } catch (e) {
  ...
  public async updateCategory(params: UpdateCategoryDto) {
    try {
      return await this.categoryRepository
          .createQueryBuilder('c')
          .update(Category)
          .set({ name: params.name, desc: params.desc, code: params.code, parentId: params.parentId, sort: params.sort})
